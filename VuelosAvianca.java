            ...
            switch (opcion) {
                ...
                case 11:
                    ...
                    if (vueloAgregarEspera != null) {
                        ...
                        System.out.println("Pasajero agregado a la lista de espera exitosamente.");
                    } else {
                        System.out.println("Vuelo no encontrado.");
                    }
                    break;
                ...
                case 12:
                    ...
                    if (vueloAsignar != null) {
                        // Asignar directamente sin verificar disponibilidad
                        Pasajero pasajeroAsignado = vueloAsignar.asignarDesdeListaEspera(scanner);
                        if (pasajeroAsignado != null) {
                            System.out.println("Pasajero " + pasajeroAsignado.nombre +
                                    " ha sido asignado desde la lista de espera.");
                        }
                    } else {
                        System.out.println("Vuelo no encontrado.");
                    }
...
class PrecioVuelos {
    private static double obtenerPrecioBase(String origen, String destino) {
        String ruta = origen + "-" + destino;

        if (ruta.equals("Bogota-Medellin")) {
            return 250000.00;
        } else if (ruta.equals("Bogota-Cali")) {
            return 230000.00;
        } else if (ruta.equals("Medellin-Cartagena")) {
            return 300000.00;
        } else if (ruta.equals("Cartagena-Bogota")) {
            return 280000.00;
        } else if (ruta.equals("Cali-Barranquilla")) {
            return 260000.00;
        } else {
            return 200000.00; // Precio por defecto
        }
    }

    public static double obtenerPrecio(String origen, String destino, String clase) {
        double precioBase = obtenerPrecioBase(origen, destino);

        if (clase.equalsIgnoreCase("BUSINESS")) {
            return precioBase * 2.5;
        } else if (clase.equalsIgnoreCase("PRIMERA")) {
            return precioBase * 3.5;
        } else {
            return precioBase * 1.0; // Economica
        }
    }
}

class HistorialReservas { //pilas
...
class ListaPasajeros { //lista enlazada
    Pasajero cabeza;
...
class Vuelo {
    ...
    private void cambiarClase(Scanner scanner, Pasajero pasajero) {
        ...
        historialReservas.agregarModificacion("Cambio de clase: " + pasajero.clase + " → " + nuevaClase +
                                                " | Asiento anterior: " + pasajero.asiento + " | Pasajero: " + pasajero.nombre);
    ...
    public Pasajero asignarDesdeListaEspera(Scanner scanner) {
        ...
        System.out.println("Asignando asiento para: " + pasajero.nombre);
        ...
        historialReservas.agregarModificacion("Asignado desde lista de espera: " +
                pasajero.nombre + " al asiento " + asiento);
...
class ArbolVuelos { //arbol binario
    NodoVuelo raiz;
